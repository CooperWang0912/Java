...
public class Rook extends Piece{
    ...
    public ArrayList<int[]> getMoves(Board board, int r, int c) {
        ...
        i = 1; //Set value to 1
        while (valid){
            if (c+i < 8 && c+i >= 0 && board.getBoard()[r][c+i].getTeam() == -1) { //If the square is in bound and empty
                int[] move = {r, c + i}; //Create move
                moves.add(move); //Add move
            }
            else if (c+i < 8 && c+i >= 0 && board.getBoard()[r][c+i].getTeam() == EneId) { //If the square is in bound and has enemy
                int[] move = {r, c + i}; //Create move
                moves.add(move); //Add move
        ...
        i = 1; //Set value to 1
        while (valid){
            if (r+i < 8 && r+i >= 0 && board.getBoard()[r+i][c].getTeam() == -1) { //If the square is in bound and empty
                int[] move = {r + i, c}; //Create move
                moves.add(move); //Add move
            }
            else if (r+i < 8 && r+i >= 0 && board.getBoard()[r+i][c].getTeam() == EneId) { //If the square is in bound and has enemy
                int[] move = {r + i, c}; //Create move
                moves.add(move); //Add move
                valid = false; //Stop the loop because the path is blocked
            }
