...
// don't edit this class
	...
	public ArrayList<int[]> getMoves(Board board, int r, int c) {
		...
		if (getTeam() == 0 && c == 1 && board.getBoard()[r][c+2].getTeam() == -
				1 && board.getBoard()[r][c+1].getTeam() == -1) {
			int[] move = {r, c+2};
			moves.add(move);
		}
		if (getTeam() == 1 && c == 6 && board.getBoard()[r][c-2].getTeam() == -
				1 && board.getBoard()[r][c-1].getTeam() == -1) {
			int[] move = {r, c-2};
			moves.add(move);
		}
...
// TODO Auto-generated method stub
	...
	public boolean check(int kingr, int kingc, int r, int c, Board board) {
		if (getTeam() == 0) {
			if (kingc - c == 1) {
				System.out.println(kingr + " " + kingc + " " + r + " " +
						c);
